...
namespace DLLClientLink.Tool
{
    class Utils
    {
        ...
        private static string autoRunName
        {
            get
            {
                return $"ClientLinkAutoRun_{GetMD5(StartupPath())}";
            }
        }
        ...
        public static string GetVersion(bool blFull = true)
        {
            try
            {
                ...
                if (blFull)
                {
                    return string.Format("ClientLink - V{0} - {1}",
                            FileVersionInfo.GetVersionInfo(location).FileVersion.ToString(),
                            File.GetLastWriteTime(location).ToString("yyyy/MM/dd"));
                }
                else
                {
                    return string.Format("ClientLink/{0}",
                        FileVersionInfo.GetVersionInfo(location).FileVersion.ToString());
                }
            }
            catch (Exception ex)
        ...
        {
            GlobalData.textLogger.WriteText(strContent);
          
        }
        public static void SaveLog(string strTitle, Exception ex)
        {
            GlobalData.textLogger.WriteText(strTitle);
        }
        ...
        public static extern bool SetProcessDPIAware(); //函数，可用于防止UI自动放大
